...
/**
 * Servlet implementation class WishDeleteSer
 */
@WebServlet("/wishlist/deletewish.do")
public class WishDeleteSer extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public WishDeleteSer() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int result=new WishlistService().deleteWish(inlist, path);
		
		if(result>0) {
			System.out.println("성공");
			response.sendRedirect(request.getContextPath()+"/wishlist/WishListEnterSer.do");
		}else {
